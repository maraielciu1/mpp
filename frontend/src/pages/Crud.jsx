...
const Crud = () => {
    const { products, addProduct, updateProduct, deleteProduct, status, fetchProducts, setProducts } = useContext(ShopContext);
    const user = JSON.parse(localStorage.getItem('activeUser'));

    const [newProduct, setNewProduct] = useState({
        image: '', name: '', description: '', price: '',
        category: '', subCategory: '', sizes: [], bestseller: false
    });
    ...
    const handleStartGeneration = async () => {
        try {
            await fetch(`${BASE_URL}/start-generation`);
